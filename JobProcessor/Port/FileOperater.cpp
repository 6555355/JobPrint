...
bool FileOperater::CreateNewFile(const char* name)
{
	char m_temp[200];  
	char dircname[200];
	memset(dircname,'\0',sizeof(dircname));
	sprintf(m_temp,name);
	int j = 0;  //Ä¿Â¼¼¶Êý
	int m_num = 0;
	for(unsigned int i = 0;i<strlen(m_temp);)
	{	
		if(m_temp[i] == '/')
		{	
			j++;
...
bool FileOperater::OpenOrCreate(const char* name)
{
	...
	std::string tempFileDir = FilePath.substr(0,pos);
	while (std::string::npos != pos)
	{
		FilePath = FilePath.substr(pos + sizeof(char));
		size_t pos = FilePath.find_first_of("\\");
		if (std::string::npos == pos)
		{
			break;
		}
		tempFileDir = tempFileDir +std::string("\\") + FilePath.substr(0,pos);
...
bool FileOperater::SeekFile(int pos, FilePOSType from)
{
	...
	if (from == FilePOSType::end) seek_from = SEEK_END;
	else if (from == FilePOSType::current) seek_from = SEEK_CUR;
