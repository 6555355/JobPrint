...
CollageBuffer::CollageBuffer(MulImageInfo_t imageInfo[], int num, double h, double w, bool isReverseData, bool isAWBMode, bool isPixel)
{
	...
	memset(AWBMapFileIdx2, 0, sizeof(int)*16*16);
...
// 				ColorNum += cNum;
// 			}
			for(int i=1; i<=AWBMapFileIdx2[0][4-1]; i++){
				int cNum;
				//cNum = header[AWBMapFileIdx[0][i]].nImageColorNum;
				cNum = header[AWBMapFileIdx2[0][i]].nImageColorNum;
				ColorNum += cNum;
			}
		}
	}
	...
	for (int i = 0; i < FileIdx.size(); i++)
	{
		...
		{/////////
			...
			for (int c = 0; c < ColorNum; c++){int colarIdx;
				...
				if(c >= FileColorNum){
					if (1)
						n = AWBMapFileIdx2[i][(c / FileColorNum - 1)];
					else
...
// 	}
	...
	{
		FileIdx.push_back(j);
		for(int i=0; i<4; i+=num/4)
		{
			if(fileInfo[i].x == fileInfo[i+1].x && fileInfo[i].y == fileInfo[i+1].y){
				vector<int> tmp;
				tmp.push_back(i);
				AWBMapFileIdx.push_back(tmp);
				AWBMapFileIdx2[j][i] = i;
			}else{
				FileIdx.clear();
				AWBMapFileIdx.clear();
				memset(AWBMapFileIdx2, 0, sizeof(int)*16*16);
