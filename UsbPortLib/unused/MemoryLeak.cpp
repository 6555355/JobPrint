...
*/
#if (defined(__SMARTJET) && defined(DEBUG_NEW)) || defined(PROJ_DEBUG_NEW)
...
DebugNewTracer::DebugNewTracer () : _lockCount (0) 
{
	...
	pthread_mutex_init(&m_metexAccessMap,NULL);
#else
...
#endif
#if defined(__SMARTJET)
	pthread_mutex_destroy(&m_metexAccessMap);
#else
...
void DebugNewTracer::Dump ()
{
	if (_map.size () != 0)
	{
		...
		for (iterator it = _map.begin (); it != _map.end (); ++it)
		{
			...
			void *p = it->second.Addr ();
			std::cout << file << ", "  << line << std::endl;
#if defined(LOG_FILE)
			fprintf(_logfp,"%s, %d,p = 0x%08x, size=%d\n", file, line, p, size);
			fflush(_logfp);
#endif
		}
	}
...
#endif//#if defined(__SMARTJET) && define(DEBUG_NEW)
