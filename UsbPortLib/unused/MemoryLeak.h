...
#else
#define LOG_FILE_NAME "c:\\debugnew.log"
#endif//#if defined(__SMARTJET)
...
private:
	...
	class Entry  {
	public:
		Entry (char const* file, int line) : _file (file), _line (line)	{_size = 0; _p = nullptr;}
		Entry (char const* file, int line, int size) : _file (file), _line (line), _size (size) {_p = nullptr;}
		Entry (char const* file, int line, int size, void *p) : _file (file), _line (line), _size (size), _p(p) {}
		
		Entry () : _file (0), _line (0), _size (0) {_p = nullptr;}
		const char* File () const { return _file; }
		int Line () const { return _line; }
		size_t Size () const { return _size; }
		void *Addr () const {return _p;}
	private:
		char const* _file;
		int _line;
		size_t _size;
 ...
 private:
	void lock () { 
#if defined(__SMARTJET)
		pthread_mutex_lock(&m_metexAccessMap);
#else
	...
	void unlock () { 
#if defined(__SMARTJET)
		pthread_mutex_unlock(&m_metexAccessMap);
#else
 ...
 private:
	...
	int _lockCount;
#if defined(__SMARTJET)
	pthread_mutex_t  m_metexAccessMap;
#else
